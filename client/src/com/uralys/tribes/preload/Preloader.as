	...
	public class Preloader extends SparkDownloadProgressBar
	{
		...
		{    
			if (!preloaderDisplay) {
				...
				var startX:Number = Math.round((stageWidth - preloaderDisplay.width + 280) / 2);
				var startY:Number = Math.round((stageHeight - preloaderDisplay.height + 280) / 2);
		...
		override protected function setInitProgress(completed:Number, total:Number):void {
			if (preloaderDisplay) {
				...
				if (completed > total) {
					...
					setPreloaderLoadingText("Loading " + completed + " of " + total);
				}
			}
		} 
